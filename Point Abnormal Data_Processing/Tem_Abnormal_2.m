% K-medoids Clustering for Temperature Anomaly Detection
% Purpose: Perform 5D clustering on temperature features and label anomalies

clear; clc;

%% 1. PARAMETER CONFIGURATION
filename = 'tem_pre.mat'; % Input feature data
Font = 12;
linewidth = 2;
...
if ~exist(filename, 'file')
    error('File not found. Ensure tem_pre.mat is in current directory.');
end
...
temperature = feature_matrix(:, 2); % Original temperature
temp_anomaly = feature_matrix(:, 3); % Temperature anomaly
temp_mean = feature_matrix(:, 4);   % Moving average
temp_std = feature_matrix(:, 5);    % Standard deviation
temp_median = feature_matrix(:, 6); % Median
temp_Q1 = feature_matrix(:, 7);     % First quartile
temp_Q3 = feature_matrix(:, 8);     % Third quartile

% Prepare feature matrix for clustering (5-dimensional)
feature_matrix_5d = [temp_anomaly, temp_mean, temp_std, temp_median, temp_Q1, temp_Q3];
...
xlabel('Temperature Anomaly', 'FontSize', Font, 'FontWeight', 'bold');
ylabel('Moving Average', 'FontSize', Font, 'FontWeight', 'bold');
zlabel('Standard Deviation', 'FontSize', Font, 'FontWeight', 'bold');
title('K-medoids Clustering Results (3D View)', 'FontSize', Font, 'FontWeight', 'bold');
...
scatter(temp_mean(normal_indices), temp_std(normal_indices), 25, 'g', 'filled', ...
        'DisplayName', 'Normal');

hold on;
% Plot anomaly points
scatter(temp_mean(anomaly_indices), temp_std(anomaly_indices), 25, 'r', 'filled', ...
        'DisplayName', 'Anomaly');

title('Temperature Data Clustering Results', 'FontSize', Font, 'FontWeight', 'bold');
xlabel('Moving Average (℃)', 'FontSize', Font, 'FontWeight', 'bold');
...
% plot(time, temperature, 'g-', 'LineWidth', 1, 'DisplayName', 'Temperature');
scatter(time, temperature, 12, 'g', 'filled','DisplayName', 'Normal');
hold on;
% Highlight anomaly points
scatter(time(anomaly_indices), temperature(anomaly_indices), 12, 'r', 'filled', ...
        'DisplayName', 'Anomaly');

xlim([min(time), max(time)]);
set(gca, 'XTick', min(time):Time_step/24:max(time));
datetick('x', 'HH:MM', 'keeplimits');
title('Temperature Data with Anomaly Detection', 'FontSize', Font, 'FontWeight', 'bold');
ylabel('Temperature (℃)', 'FontSize', Font, 'FontWeight', 'bold');
legend('show', 'FontSize', Font);
grid on;
hold off;
...
tem_al = [feature_matrix, idx]; % Add cluster labels as 9th column
...
if save_choice == 1
    save('tem_al.mat', 'tem_al');
    fprintf('Data saved successfully as tem_al.mat\n');
    fprintf('Cluster labels stored in column 9 of tem_al matrix\n');
else
    fprintf('Data not saved.\n');
end

fprintf('Clustering analysis completed successfully.\n');
